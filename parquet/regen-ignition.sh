...
THRIFT_DIR="$SOURCE_DIR/../../../ignition-parquet"

COMMENT='//! See [`crate::file`] for easier to use APIs.'

# Note: add argument --platform=linux/amd64 to run on mac
docker run  -v $SOURCE_DIR:/thrift -v $THRIFT_DIR:/thrift_def -it archlinux /bin/bash -c "\
  pacman -Sy --noconfirm wget thrift && \
  thrift --gen rs /thrift_def/parquet.thrift && \
